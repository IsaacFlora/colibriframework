...
class CO_limitelista{
	
	
	//Select pronto em html
	public $limitelista;
	protected $CO;//instancia do super objeto
	...
	*/
	...
	public function montaLista( $urlprincipal, $classe='', $id='', $limiteinicial=10 ){
		...
		$parametros= $this->CO->url->parametros;//RECUPERA OS PARAMETROS EXISTENTES
		
		$limiteatual= $this->CO->url->obterParametro( 'limite' );
		...
		//PAGINACAO
		$paginacaoatual= $this->CO->url->obterParametro('paginacao');//ARMAZENA PAGINACAO ATUAL
		...
		foreach( $valores as $valor ){
			...
			$limitelista.='<option '.$selected.' value="'.$urlprincipal.'/'.$this->CO->roteador->controlador.'/'.$this->CO->roteador->acao.'/'.$stringparametros.'limite/'.$valor.$paginacao.'">'.$valor.'</option>';
		}
		$limitelista.='</select>';
